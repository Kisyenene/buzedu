...
**buzeduentreprise is an opinionated, full-stack, JavaScript/TypeScript web application
framework designed to keep you moving fast as your app grows from side project
to startup.**

At the highest level, a buzeduentreprise app is a React frontend that talks to a custom
...
off, buzeduentreprise's architecture allows you to deploy to either serverless providers
...
By making a lot of decisions for you, buzeduentreprise lets you get to work on what makes
your application special, instead of wasting cycles choosing and re-choosing
various technologies and configurations. Plus, because buzeduentreprise is a proper
framework, you benefit from continued performance and feature upgrades over time
and with minimum effort.

buzeduentreprise is the latest open source project initiated by Tom Preston-Werner,
...
> **TUTORIAL:** The best way to get to know buzeduentreprise is by going through the
> extensive [buzeduentreprise Tutorial](https://buzeduentreprisejs.com/docs/tutorial). Have fun!

> **QUICK START:** You can install and run a full-stack buzeduentreprise application on
> your machine with only a couple commands. Check out the [Quick
> Start](https://buzeduentreprisejs.com/docs/quick-start) guide to get started.

**EXAMPLES:** If you'd like to see some simple examples of what a buzeduentreprise
application looks like, take a look at the following projects:

- [Todo](https://github.com/buzeduentreprisejs/example-todo)
- [Store](https://github.com/buzeduentreprisejs/example-store-stripe)
- [Invoice](https://github.com/buzeduentreprisejs/example-invoice)
...
Here's a quick taste of the technologies a standard buzeduentreprise application will
use:
...
- First-class Jamstack-style deployment to both serverless and traditional infrastructure: [Netlify](https://www.netlify.com/), [Vercel](https://vercel.com/), [Serverless](https://www.serverless.com/), [Render](https://render.com/), [Docker container](https://community.buzeduentreprisejs.com/t/dockerize-buzeduentreprisejs/2291) (for AWS, Google Cloud, Azure, etc.), and many more on the way!

## How it works

A buzeduentreprise application is split into two parts: a frontend and a backend. This is
...
called both from external GraphQL requests and other internal services. buzeduentreprise
...
The web side is built with React. buzeduentreprise's router makes it simple to map URL
...
buzeduentreprise utility components make it trivial to implement smart forms and various
common needs. An ideal development flow starts with Storybook entries and Jest
tests, so buzeduentreprise closely integrates both, making it easy to do the right thing.

You'll notice that the web side is called "web" and not "frontend". This is
because buzeduentreprise conceives of a world where you may have other sides like
"mobile", "desktop", "cli", etc., all consuming the same GraphQL API and living
in the same monorepo.

## Roadmap

A framework like buzeduentreprise has a lot of moving parts; the Roadmap is a great way
...
great way to get involved! [Roadmap](https://buzeduentreprisejs.com/roadmap)

## Why is it called buzeduentreprise?

_(A history, by Tom Preston-Werner)_

Where I live in Northern California there is a type of tree called a buzeduentreprise.
buzeduentreprises are HUGE, the tallest in the world, some topping out at 115 meters (380
feet) in height. The eldest of the still-living buzeduentreprises sprouted from the
...
grove of buzeduentreprises and walk among these giants, soaking myself in their silent
grandeur.

In addition, buzeduentreprises have a few properties that I thought would be aspirational
for my nascent web app framework. Namely:

- **buzeduentreprises are beautiful as saplings, and grow to be majestic.** What if you
  could feel that way about your web app?

- **buzeduentreprise pinecones are dense and surprisingly small.** Can we allow you to
  get more done with less code?

- **buzeduentreprise trees are resistant to fire.** Surprisingly robust to disaster
  scenarios, just like a great web framework should be!

- **buzeduentreprises appear complex from afar, but simple up close.** Their branching
...
## Contributors
*A gigantic "Thank YOU!" to everyone below who has contributed to one or more buzeduentreprise projects: [Framework](https://github.com/buzeduentreprisejs/buzeduentreprise), [Website](https://github.com/buzeduentreprisejs/sprout), [Docs](https://github.com/buzeduentreprisejs/buzeduentreprise/tree/main/docs), and [Create-buzeduentreprise Template](https://github.com/buzeduentreprisejs/buzeduentreprise/tree/main/packages/create-buzeduentreprise-app/template). 🚀*
...
buzeduentreprise projects *(mostly)* follow the [all-contributions](https://allcontributors.org/) specification. Contributions of any kind are welcome.
